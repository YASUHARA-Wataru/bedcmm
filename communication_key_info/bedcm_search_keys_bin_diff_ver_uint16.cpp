...
struct BinaryDatas {
    ...
    // 調査中の基底以外の論理和をとった場合
    uint16_t pre_logical_or(int Bit_length) {
        uint16_t acc = 0;
    ...
    // 全ての論理和をとった場合
    uint16_t logical_or(uint16_t pre_data ,int Bit_length) {
        uint16_t acc = 0;
...
int main(void)
{
    ...
    while (bases.loadBinary(ifs)) {
        ...
        pre_logical_or_data = bases.pre_logical_or(length);
        for(uint16_t i = last_num+1; i < max_length; i++){
            ...
            for(int k = 0; k < baseCount ; k++){
                cor_data = bases.logical_or(pre_logical_or_data,length);
        ...
        if (done % (total/100 == 0 ? 1 : total/100) == 0) { // 1%刻み
