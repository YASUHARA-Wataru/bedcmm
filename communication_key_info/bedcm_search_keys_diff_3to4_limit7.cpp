...
int main(void)
{
	...
	while(std::getline(readingfile, raeding_line_buffer)){
		...
		for(int i=bases_num[baseCount-2]+1;i<max_number+1;++i){
			...
			// get corr data
			for (int j = 0; j < 2 * n; ++j) {
				send_sig1[j] = bases[0][j % n] | bases[1][j % n] | bases[2][j % n] | bases[3][j % n];
			}
			...
			if (valid) {
				...
				for (int j = 0; j < 2 * n; ++j) {
					send_sig2[0][j] = bases[1][j%n] | bases[2][j%n] | bases[3][j%n];
					send_sig2[1][j] = bases[0][j%n] | bases[2][j%n] | bases[3][j%n];
					send_sig2[2][j] = bases[0][j%n] | bases[1][j%n] | bases[3][j%n];
					send_sig2[3][j] = bases[0][j%n] | bases[1][j%n] | bases[2][j%n];
				}
